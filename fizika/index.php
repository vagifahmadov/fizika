...
<!-- RESPONSIV MENU --><link href="responsiv.css" rel="stylesheet" media="screen" />
...
sehifeerror="success";
	...
	$(document).ready(function(){
		...
		function responsiv_sehife(){
			//responsiv
			if ($( window ).width() < 917){
				$('.basliq').hide();
				$('.body').css('background-image','url(img/responsivbg.jpg)');
				$('.slayd').hide();
				$('.esas').css('margin-top','50px');
				$('.main').css('width','100%');
				$('.bas_balacaa').show();
				$('.orta').css('margin-top','-40px');
			}else if ($( window ).width() > 917 && $( window ).width() < 1041){
				$('.basliq').show();
				$('.body').css('background-image','url(img/bg_fizika.jpg)');
				$('.slayd').hide();
				$('.esas').css('margin-top','0px');
				$('.orta').css('margin-top','20px');
				$('.main').css('width','100%');
				$('.bas_balacaa').hide();
			}else {
				$('.basliq').show();
...
?>
      ...
      <?php while($altmenu=mysql_fetch_array($alt_m)){
		  ...
		  ?>
      <li <?php echo $klass_menu_m; ?>><a <?php if(mysql_num_rows($alt_alt_m)==0){ ?> href="index.php?id=<?php echo $altmenu['id']."&alt_m=".$altmenu['alt_m']; ?>"<?php } ?> ><span><?php echo $altmenu['menu']; ?></span></a>
...
<table border="0" align="center" cellpadding="0" cellspacing="0">
   <tr>
    ...
    <?php
		...
		$alt_m=@mysql_query("select * from menu where alt_m='".$sutun_menu['id']."' ORDER BY 'sira_n'", $elaqe);
?>
        <li id="menu_teyin<?php echo $sutun_menu['id']; ?>"><a <?php if(mysql_num_rows($alt_m)==0){ ?>href="index.php?id=<?php echo $sutun_menu['id']."&alt_m=".$sutun_menu['alt_m']; ?>"<?php } ?> <?php if(mysql_num_rows($alt_m)>0){ ?>class="parent"<?php } ?>><span><?php echo $sutun_menu['menu']; ?></span></a>
		<?php if(mysql_num_rows($alt_m)>0){ ?>
      <ul>
      <?php while($altmenu=mysql_fetch_array($alt_m)){
		  		$alt_alt_m=@mysql_query("select * from menu where alt_m='".$altmenu['id']."' ORDER BY 'sira_n'", $elaqe);
		  ?>
      <li><a <?php if(mysql_num_rows($alt_alt_m)==0){ ?>href="index.php?id=<?php echo $altmenu['id']."&alt_m=".$altmenu['alt_m']; ?>"<?php } ?> <?php if(mysql_num_rows($alt_alt_m)>0){ ?>class="parent"<?php } ?>><span><?php echo $altmenu['menu']; ?></span></a>
	  <?php if(mysql_num_rows($alt_alt_m)>0){ ?>
                    <ul>
                  <?php while($alt_altmenu=mysql_fetch_array($alt_alt_m)){ ?>
                        <li><a href="index.php?id=<?php echo $alt_altmenu['id']."&alt_m=".$altmenu['alt_m']; ?>"><span><?php echo $alt_altmenu['menu']; ?></span></a></li>
...
<?php 
//menu sehife yolu
		$kontent_sorgu="select * FROM menu where id=".$_GET['id'];
		$kontent=mysql_fetch_array(mysql_query($kontent_sorgu, $elaqe));
		
		
		$yolteyin=@mysql_query("select * from menu where id='".$_GET['id']."'", $elaqe);
		if(mysql_num_rows($yolteyin)>0){
			$yolteyinnetice=mysql_fetch_array($yolteyin);
		}
		
		if($yolteyinnetice['alt_m']!=0){
			//echo $kontent['menu'];
			$yolteyin2=@mysql_query("select * from menu where id='".$yolteyinnetice['alt_m']."'", $elaqe);
			if(mysql_num_rows($yolteyin2)>0){
			$yolteyinnetice2=mysql_fetch_array($yolteyin2);
			}
			
			if($yolteyinnetice2['alt_m']==0){
				echo "<div class='ara_unvan'>".$yolteyinnetice2['menu']." > ".$kontent['menu']."</div>";
			}else{
				$yolteyin3=@mysql_query("select * from menu where id='".$yolteyinnetice2['alt_m']."'", $elaqe);
				if(mysql_num_rows($yolteyin3)>0){
				$yolteyinnetice3=mysql_fetch_array($yolteyin3);
				}
				
				if($yolteyinnetice3['ara_unvan']==0){
					echo "<div class='ara_unvan'>".$yolteyinnetice3['menu']." > ".$yolteyinnetice2['menu']." > ".$kontent['menu']."</div>";
				}
			}
		}
		?>
...
<script>
    ...
    //fixed header basliq
     $(window).scroll(function() {
        if ($(this).scrollTop() > 203 && $( window ).width() > 934){
            $('.bas_balacaa').show();
            $('.bas_balacaa').animate({top: '70px'});
          }
          else if ($(this).scrollTop() < 203 && $( window ).width() > 934){
            $('.bas_balacaa').hide();
    ...
    //responsiv sehife
    $( window ).resize(function() {
		/*if ($( window ).width()< 953 && $( window ).width()> 933) {
